# -*- coding: utf-8 -*-
"""
Created on Mon May  8 13:35:54 2023
...
def err_ranges(x, y, popt, pcov, alpha=0.05):
    '''Provides us with the confidence interval for the data '''
    n = len(y)
    p = len(popt)
...
y_fit = func(x, *popt)
y_lower, y_upper = err_ranges(x, y, popt, pcov)  # lower and upper range

# plot the fit curve and 95% confidence interval
y_fit = func(x, *popt)  # plot
sigma = np.sqrt(np.diag(pcov))  # sigma value
y_lower = func(x, *(popt - 1.96 * sigma))  # lower range
y_upper = func(x, *(popt + 1.96 * sigma))  # upper range
...
ax.bar(range(len(means)), means, yerr=stds,
       tick_label=means.index)  # set the range to plot
...
fig, ax = plt.subplots()
ax.scatter(data_norm["GDP_per_capita"],
           data_norm["Energy_use_per_capita"])  # set the data range
ax.set_xlabel("GDP per capita (PPP, inflation-adjusted)")  # set the x label
ax.set_ylabel("Energy use per capita")  # set the y label
# set the title
ax.set_title("Scatter plot of GDP per capita and energy use per capita")
# Set the date range
...
co2_data = data_countries.pivot_table(
    index='date', columns='country', values='CO2_emissions')
# Size of the figure
...
gdp_data = data_countries.pivot_table(
    index='date', columns='country', values='GDP_per_capita')
...
data_countries = wbdata.get_dataframe({"EG.USE.PCAP.KG.OE": "Energy_use_per_capita"}, country=[
                                      'USA', 'CAN', 'MEX', 'GBR', 'IND'], data_date=dates)
# Remove the missing data
data_countries = data_countries.dropna()
# Pivot the data to get energy use per capita per country per year
energy_data = data_countries.pivot_table(
    index='date', columns='country', values='Energy_use_per_capita')
...
fig, ax = plt.subplots()
ax.pie(cluster_counts, labels=["Low GDP, Low CO2 emissions",
       "Medium GDP, Medium CO2 emissions", "High GDP, High CO2 emissions"], autopct="%1.1f%%")
# set the title
ax.set_title("Percentage of Countries by GDP and CO2 Emissions Cluster")
plt.show()  # show the plot

data_countries = wbdata.get_dataframe({"EN.ATM.CO2E.KT": "CO2_emissions"}, country=[
                                      "USA", "CAN", "MEX", "GBR", "IND"])
...
# Pivot the data to get CO2 emissions per country per year
co2_data = data_countries.pivot_table(
    index="country", columns="date", values="CO2_emissions")
